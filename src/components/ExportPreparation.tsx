...
export function ExportPreparation({ result, templates, organizationId, onSave }: ExportPreparationProps) {
  ...
  return <section className="panel export-preparation" aria-labelledby="prepare-export-title">
    ...
    {workflow.mode === "template" && <div className="template-source-actions">
      ...
      {templateSelected && <div className="template-configuration"><div><h3>Template structure</h3><p>{draft.destinationType || "No destination label"}{draft.sheetName ? ` · Worksheet: ${draft.sheetName}` : ""}</p></div><ol>{draft.columns.map((column) => <li key={column.id}><strong>{column.header || "(blank header)"}</strong><span>{column.source.kind === "parameter" ? `Runtime value: ${column.source.label}` : column.source.kind === "constant" ? "Constant" : column.source.kind === "empty" ? "Empty" : "Mapped field"}</span></li>)}</ol></div>}
    </div>}
