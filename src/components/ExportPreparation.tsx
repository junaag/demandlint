...
export function ExportPreparation({ result, templates, organizationId, onSave }: ExportPreparationProps) {
  ...
  function selectTemplate(id: string) {
    ...
    setSelectedTemplateId(id); setChangingTemplate(false);
  ...
  async function download() {
    ...
    try { await downloadTemplateExport(result, { ...draft, defaultFormat: format }, parameters, format); setMessage({ kind: "success", text: `${result.ready.length} ready rows exported with '${draft.name}'.` }); } catch (caught) { setMessage({ kind: "error", text: caught instanceof Error ? caught.message : "The export could not be created." }); } finally { setBusy(null); }
  }
