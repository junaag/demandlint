...
function SourceFieldControl({ column, updateColumn }: { column: ExportTemplateColumn; updateColumn: (update: (column: ExportTemplateColumn) => ExportTemplateColumn) => void }) {
  if (column.source.kind !== "canonical" && column.source.kind !== "custom") return null;
  const selection = column.source.kind === "canonical" ? `canonical:${column.source.field}` : "custom";
  return <label className="source-field-selection"><span>Source field</span><small>Select a source field from the imported dataset.</small><div className="source-field-control"><select value={selection} onChange={(event) => updateColumn((current) => {
    const value = event.target.value;
    return value === "custom" ? { ...current, source: { kind: "custom", key: current.source.kind === "custom" ? current.source.key : "" } } : { ...current, source: { kind: "canonical", field: value.slice("canonical:".length) as CanonicalField } };
  })}><option value="custom">{column.source.kind === "custom" && !column.source.key ? "Select a source field…" : "Other imported field…"}</option>{CANONICAL_FIELD_OPTIONS.map((option) => <option key={option.value} value={`canonical:${option.value}`}>{option.label}</option>)}</select>{column.source.kind === "custom" && <input value={column.source.key} placeholder="Other imported field name" onChange={(event) => updateColumn((current) => ({ ...current, source: { kind: "custom", key: event.target.value } }))} />}</div></label>;
}
  ...
  return <div className="export-template-editor"><div className="export-template-meta">
    ...
    {template.columns.map((column, index) => {
      ...
      return <article className="export-column-card" key={column.id}><header className="export-column-card-header"><div className="column-order"><strong>{index + 1}</strong><button className="column-action-button" type="button" aria-label="Move column up" disabled={index === 0} onClick={() => moveColumn(index, -1)}>↑</button><button className="column-action-button" type="button" aria-label="Move column down" disabled={index === template.columns.length - 1} onClick={() => moveColumn(index, 1)}>↓</button></div><div className="column-card-actions"><button className="column-action-button danger" type="button" aria-label={`Remove ${column.header}`} onClick={() => onChange({ ...template, columns: template.columns.filter((item) => item.id !== column.id) })}>×</button></div></header><div className="export-column-fields">
        ...
        <SourceFieldControl column={column} updateColumn={update} />
