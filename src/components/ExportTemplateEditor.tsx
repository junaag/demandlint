...
export function ExportTemplateEditor({ template, onChange, heading = "Template configuration" }: ExportTemplateEditorProps) {
  ...
  function requiredField(column: ExportTemplateColumn) {
    ...
    return <label className="required-toggle"><span>Required for export</span><small>Export is blocked if this column has no value after applying defaults.</small><input type="checkbox" checked={column.required ?? false} onChange={(event) => updateColumn(column.id, (current) => ({ ...current, required: event.target.checked }))} /></label>;
  }
