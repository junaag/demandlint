...
export function ExportTemplatesPage({ templates, organizationId, onSave, onDelete }: ExportTemplatesPageProps) {
  ...
  return <section className="templates-page">
    ...
    <div className="templates-page-layout">
      <section className="panel templates-list-panel"><div className="section-heading"><div><p className="section-label">WORKSPACE TEMPLATES</p><h2>{templates.length} template{templates.length === 1 ? "" : "s"}</h2></div><button className="button primary" type="button" onClick={createTemplate}>Create template</button></div>
      ...
      {draft && <section className="panel template-editor-panel"><div className="section-heading"><div><p className="section-label">{templates.some((template) => template.id === draft.id) ? "EDIT TEMPLATE" : "NEW TEMPLATE"}</p><h2>{draft.name || "Untitled template"}</h2></div><button className="text-button" type="button" onClick={() => setDraft(null)}>Close</button></div><ExportTemplateEditor template={draft} onChange={setDraft} /><div className="template-editor-actions"><button className="button primary" type="button" disabled={busy !== null || !draft.name.trim()} onClick={() => void saveDraft()}>{busy === "save" ? "Saving…" : "Save template"}</button></div></section>}
    </div>
  </section>;
}
